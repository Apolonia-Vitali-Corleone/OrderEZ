...
func NewGoodHandler(goodService *service.GoodService) *GoodHandler {
	return &GoodHandler{goodService: goodService}
}

// GetAllGoods 方法用于获取指定页码和每页数量的商品
func (h *GoodHandler) GetAllGoods(c *gin.Context) {
	...
	goods, err := h.goodService.GetAllGoods(page, pageSize)
	if err != nil {
