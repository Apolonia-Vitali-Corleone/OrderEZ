...
func (s *GoodService) GetAllGoods(page, pageSize int) ([]model.Good, error) {
	// 尝试从 Redis 缓存中获取商品列表
...
func (s *GoodService) getGoodFromCache(key string) (model.Good, error) {
	val, err := s.redis.Get(s.redis.Context(), key).Result()
	if err != nil {
		if err == redis.Nil {
...
func (s *GoodService) setGoodInCache(key string, good model.Good) error {
